...
void Jeu::lancementJeu() {
    ...
    std::list<std::map<Carte*, int>> save;
    int tourNb = 0;
    ...
    if (inFile.is_open()) {
        inFile >> tourNb;
        std::map<Carte*, int> myMap;
        Carte* carte;
        int value;

        while (inFile >> carte >> value) {
            myMap[carte] = value;
            char newline;
            inFile.get(newline);
            if (newline == '\n') {
                listOfMaps.push_back(myMap);
                myMap.clear();
            }
        }
    ...
    tourNb = tour(tourNb);//pour l'instant pas d'enregistrement donc le jeu commence au joueur 0


    std::ofstream outFile("save.txt");
    if (outFile.is_open()) {
        outFile << tourNb << std::endl;
        for (const auto& myMap : save) {
        ...
        std::cerr << "Fichier de save introuvable." << std::endl;
    }
